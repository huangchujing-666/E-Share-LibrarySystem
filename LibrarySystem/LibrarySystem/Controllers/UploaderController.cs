...
namespace Exam.Admin.Controllers
{
    ...
    public class UploaderController : BaseController
    {
        ...
        public UploaderController(IBaseImageService baseImageService)
        {
            _baseImgInfoService = baseImageService;
        }
        /// <summary>
        /// 视频文件上传,返回视频地址
        /// </summary>
        ...
        public JsonResult ImgFile(string folder = "ImgUploader", string title = "title")
        {
            lock (imgLock)
            {
                ...
                return Json(entity.BaseImageId, JsonRequestBehavior.AllowGet);
            }
        }
        /// <summary>
        /// 图片上传压缩
        ...
        public JsonResult DeleteFile(int id = 0)
        {
            ...
            //string serverPath = Server.MapPath(Path.Combine(info.Path));
        ...
        public JsonResult GetImgInfos(string ids = "")
        {
            ...
            var list = _baseImgInfoService.GetAll().Where(p=> ids.Contains(p.BaseImageId.ToString()));
